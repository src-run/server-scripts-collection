...
#
# output (write) styled text using bright library and printf notation
#
function write_styled_text {
    _out "${@}"
}
...
#
function write_normal_line {
    local format="${1}"
    shift

    write_normal_text "${format}" "${@}"
    write_newline
}
...
# output (write) styled text using bright library and printf notation
#
function write_styled_line {
    _out 'ctl:use-nl' 'ctl:use-interpret' 'ctl:auto-reset' "${@}"
}
...
function write_error {
    ...
    write_styled_text " ERROR: ${header} " \
        'fg:red bg:black style:bold style:reverse' >&2
    write_styled_text " ${error}" \
        'fg:red style:bold' >&2
    write_newline
}
...
function write_usage_header {
    ...
    write_styled_line "${header^^}" 'style:bold'
}
...
function write_usage_action {
    ...
    write_normal_text "  ${action_name:0:1}  $(printf '%-16s' "${action_long}")"
    write_styled_line "${action_desc}" 'style:dim'
}
...
function write_usage_arg {
    ...
    write_normal_text "  -${arg_name:0:1} --$(printf '%-14s' "${arg_long_typed}")"
    ...
    for line in "${@}"; do
        ...
        write_styled_line "${line}" 'style:dim'
        num_desc=$((${num_desc} + 1))
    done
}
...
function write_usage {
    ...
    write_normal_line '  ./%s ARGUMENTS [ACTION CONTEXT]' "$(get_script_name)"
...
function do_action_attach {
    ...
    write_normal_line 'Device "%s" (%s) has been attached to "%s".' \
        "${device_id_slave}" \
        "${context}" \
        "${device_id_master}"
...
function do_action_detach {
    ...
    write_normal_line 'Device "%s" (%s) has been detached from "%s".' \
        "${device_id_slave}" \
        "${context}" \
        "${device_id_master}"
}
...
function do_action_status {
    ...
    if [[ $? -ne 0 ]]; then
        write_normal_line 'Device "%s" (%s) is ATTACHED to "%s".' \
            "${device_id_slave}" \
            "${context}" \
            "${device_id_master}"
    else
        write_normal_line 'Device "%s" (%s) is DETACHED.' \
            "${device_id_slave}" \
            "${context}"
    fi
}
